...
- **只看整体缺口：**阅读“第一部分”的第 1 节，快速判断哪些大模块未覆盖、部分覆盖、待核对或覆盖较完整。
- **查具体 BVT 目录：**阅读“第一部分”的第 2 节，72 个顶层目录均只出现一次，可直接查看对应状态和剩余缺口。
- **核对判定依据：**阅读“第二部分”，其中包含功能拆分、详细缺口、补充优先级、版本基线和证据链接。
...
### 2. BVT 72 个目录覆盖状态清单（查具体目录）

以下逐项列出 `test/distributed/cases` 的全部 72 个顶层目录。文件数只统计 `.sql/.test`；每个目录只出现一次，总计 1109 个文件。

状态含义：**未覆盖**表示其他回归未找到直接测试；**部分覆盖**表示只覆盖主链路、仍有明确缺口；**待逐项核对**表示已有相近场景但尚未完成 case 对照；**覆盖较完整**表示已有直接专项。

#### 2.1 SQL 函数、类型与表达式（9 个目录）
...
`function`、`dtype`、`operator` 和 `expression` 已有场景覆盖，归入“部分覆盖”，见 5.2。
